...
export const mockStationWeather = {
  2206: {
    ...
    min_temp: 29.5,
    weather_description:
      "Localized thunderstorms and lightning with light rain at scattered locations",
  },
  2500: {
    max_temp: 23.9,
    min_temp: 22.4,
...
export const buildDailyForecast = (district) => {
  const data =
    districtForecastData[district] || districtForecastData[DEFAULT_DISTRICT];
...
export const CLIMATE_SCOPE_LABEL =
  "Nagaland-wide averages, not specific to your selected district";
...
const createHourlyData = (dayIndex, districtSeed) =>
  Array.from({ length: 24 }, (_, hour) => {
    const displayHour = hour % 12 === 0 ? 12 : hour % 12;
    const period = hour < 12 ? "AM" : "PM";
    const seed = districtSeed + dayIndex * 24 + hour;
...
export const buildForecastDays = (district) => {
  const districtSeed = seedFromName(district || DEFAULT_DISTRICT);
  const today = new Date();

  return Array.from({ length: 8 }, (_, i) => {
    const date = new Date(today);
    date.setDate(today.getDate() + i);
    const label = i === 0 ? "Today" : WEEKDAY_NAMES[date.getDay()];
    const seed = districtSeed + i * 9;
    ...
    return {
      ...
      high: 26 + (seed % 8),
      low: 17 + (seed % 6),
      hourly: createHourlyData(i, districtSeed),
    };
  });
};
